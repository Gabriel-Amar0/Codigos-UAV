...
NS_LOG_COMPONENT_DEFINE ("UavSim1-2");
...
main (int argc, char *argv[])
{
  ...
  // Se o destino for o mesmo ponto inicial, o UAV permanece estático
  if (length < 0.001)  // diferença praticamente zero
  {
    mob->SetVelocity(Vector(0.0, 0.0, 0.0));
    NS_LOG_UNCOND("UAV permanece estático (sem destino definido).");
  }
  else
  {
    ...
    NS_LOG_UNCOND("UAV movendo-se em direção ao destino definido.");
  }
  ...
  ofstream logFile ("scratch/UavSim1-2-D" + to_string((int)distance) + "-T" + to_string((int)simTime) + ".log", ios::out);
